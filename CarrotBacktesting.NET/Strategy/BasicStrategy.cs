    ...
    public class BasicStrategy : IStrategy
    {
        ...
        public void OnNext(StrategyContext strategyContext)
        {
            ...
            if (strategyContext.NowPrice <= 3)
                strategyContext.PortfolioManager.AddOrder(new GeneralOrder("A", "001", 2.5, 100.0, PositionDirectionEnum.Long));
            if (strategyContext.NowPrice >= 6)
                strategyContext.PortfolioManager.AddOrder(new GeneralOrder("A", "001", 4.5, 100.0, PositionDirectionEnum.Short));
        }
    }
}
